...
import { CalendarDays, Users, TrendingUp, Image } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const [sns, setSns] = useState({ total: 0, done: 0 })
  const now = new Date()
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: ev }, { data: mem }, { data: vis }, { data: snsData }] = await Promise.all([
        ...
        supabase.from('sns_content').select('status').gte('post_date', `${thisYear}-${String(thisMonth).padStart(2,'0')}-01`),
      ])
      setEvents(ev || [])
      setMembership(mem)
      setVisits((vis || []).reduce((s, r) => s + (r.visitor_count || 0), 0))
      setSns({ total: (snsData || []).length, done: (snsData || []).filter(r => r.status === '업로드완료').length })
    }
    load()
  }, [])
  ...
  const stats = [
    { label: '이번 달 1개월권', value: `${membership?.monthly_pass_count ?? '-'}명`, icon: Users, color: 'text-brand' },
    { label: '이번 달 4시간권', value: `${membership?.four_hour_pass_count ?? '-'}회`, icon: TrendingUp, color: 'text-blue-600' },
    { label: '이번 달 1일권 방문', value: `${visits}회`, icon: CalendarDays, color: 'text-orange-500' },
    { label: 'SNS 이번 달 업로드', value: `${sns.done}/${sns.total}건`, icon: Image, color: 'text-purple-600' },
  ]
